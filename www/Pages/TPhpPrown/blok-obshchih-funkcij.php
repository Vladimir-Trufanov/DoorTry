...
<p>Две функции <span class="letter">getComRequest</span>, <span class="letter">isComRequest</span> 
позволяют выбирать или проверять значения параметров в GET или POST запросах, 
направляемых к сайту. По умолчанию предполагаются для проверки или выборки, 
так называемые команды в запросе к сайту, то есть параметры с именем "Com". 
Например, в запросе для сайта kwinflat.ru "показать справочник услуг", 
который выглядит следующим  образом: 
<strong><a href='https://kwinflat.ru/Main.php?Com=Uslugi' target='_blank'>https://kwinflat.ru/Main.php?Com=Uslugi</a></strong>, 
параметр "Com" имеет  значение "Uslugi".</p>
<p>ЗАМЕЧАНИЕ: При одноименном кукисе на сайте, значение кукиса имеет приоритет 
над параметром POST.</p>

<p>Функция <span class="letter">getPhpVersion</span> формирует пятиразрядное 
число, указывающее на текущую версию интерпретатора PHP. Используется для 
управления кодом проекта на PHP в зависимости от версии.</p>
 
<p>Функции <span class="letter">NoSpace</span> и <span class="letter">NoZero</span> 
заменяют вывод околонулевых значений, то есть маленьких чисел в диапазоне 
[-0.0001, 0.0001] на пробел. Функция NoSpace дополнительно выводит указанное 
сообщение, если число вне диапазона околонулевых значений.</p>

<p>Функция <span class="letter">sayLogic</span> преобразовывает логическое 
значение в соответствующий текст.</p>

<p>Функции <span class="letter">makeLabel</span> и <span class="letter">isLabel
</span> позволяют передавать и обнаруживать сообщения, содержащие заданную 
метку. Метка - это фрагмент строки, ограниченный с левой и правой стороны 
некоторыми наборами контрольных символов. Функция makeLabel задает метку, а 
функция isLabel вылавливает метку в переданном текстовом сообщении.</p>
